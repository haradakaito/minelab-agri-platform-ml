...
if __name__ == '__main__':
    try:
        ...
        for field_device in config["FieldDevice"]["Image"]:
            Util.create_path(path=f'{Util.get_root_dir()}/raw-data/image-data/{field_device}')
            # 指定した時間範囲のオブジェクトをダウンロード
            aws_handler.download_s3_objects(
                remote_path = f'projects/csi/image-data/{field_device}/',                   # S3のプレフィックス
                local_path  = f'{Util.get_root_dir()}/raw-data/image-data/{field_device}/', # ローカルパス
